...
import {ChangeFollow, ClearAllUsers, thunkGetCategory, thunkGetUsers} from "../../../redux/allPosts-reducer";
...
let PostCreator = (props) =>{
    ...
    console.log(props)
    return <>
        {props.allPosts.posts.length!==0?
            props.allPosts.posts.map((el)=>{
                return <div className={s.card}>
                    <div className={s.category}>
                        <div>{el.type.rusName}</div>
                    </div>
                    <div className={s.img}>
                        <img src={el.avaImg} alt=""/>
                    </div>
                    <div className={s.content}>
                        ...
                        <div  className={s.also}>
                            <div className={s.time}>
                                ...
                                <NavLink to={'/CMS/CreatePostPage/'+el.id}>
                                    <button>
                                        <svg id="edit" xmlns="http://www.w3.org/2000/svg" width="25.77" height="25.64" viewBox="0 0 25.77 25.64">
        ...
        return <>
            ...
            <div>
                ...
                <div className={s.gridCategori}>
                    {this.props.allPosts.category.map(el=><div onClick={()=>{this.clickOnCategory(el.id)}} className={el.isFollow?(s.el+' '+s.active):s.el}>
                        {el.rusName}
                    </div>)}
                </div>
            </div>
...
export default compose(
    connect(mapStateToProps, {thunkGetUsers,thunkGetCategory, ClearAllUsers, ChangeFollow}),
)(AllPosts)