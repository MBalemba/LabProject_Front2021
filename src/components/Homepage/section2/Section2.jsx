            ...
            <div className={s.newsBlock}>
                {props.posts.map((el, index) => {
                    return <div className={s[`child${index + 1}`]}>
                        <NewContent post={props.posts[index]}/>
                    </div>
                })}
            </div>
        </>
...
const NewContent = (props) => {
    ...
    return (
        <div className={s.Blockingrid}>
            ...
            <img src={props.post.avaImg} alt=""/>
            <div className={s.contentBlock}>
                <div></div>

                <div>
                    <NavLink onClick={(e) => {
                        if (props.post.type.pathName === 'default') e.preventDefault()
                    }} to={`/new/${props.post.id}`}><h2>{props.post.title}</h2></NavLink>
                    {props.post.type.pathName !== 'default' ?
                        <div className={s.time}>
                            ...
                            <p className={s.data}>{props.post.data.day + ' ' + props.post.data.month + ' ' + props.post.data.year}</p>
